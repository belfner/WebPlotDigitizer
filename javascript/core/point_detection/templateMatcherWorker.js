...
// The body of the template matcher worker. It is bundled into combined.js / wpd.min.js
// (and loaded as a normal script in dev mode) so it ships with the rest of the code.
// wpd.TemplateMatcherAlgo stringifies this function and runs it from a Blob URL, which
// is what actually executes in the Worker context (self.onmessage below). Keep the body
// self-contained: it has no access to the wpd namespace once running inside the Worker.
wpd.templateMatcherWorkerBody = function() {
    ...
    self.onmessage = function(m) {
        ...
        self.postMessage(matchTemplate(
            data.templ,
            data.matchThreshold,
            data.binaryData,
            data.imageWidth,
